...
public class AlarmSendingThread extends AbstractTerminationThread {
    ...
    public AlarmSendingThread() {
        ...
        System.out.println("后台线程创建了存储告警信息的队列和记录告警信息提交的次数");
    }

    /**
     * 发送机器告警信息，返回告警信息提交过的次数；-1： ，n：之前提交过的次数
     * 1.报警管理器终止，直接返回异常。
     * 2.报警管理器运行中，某业务种类告警信息第一次提交，放入告警信息队列+1
     * 3.某业务种类告警信息之前提交过，记录次数+1
     *
    ...
    public int sendAlarm(AlarmInfo alarmInfo) {
        ...
        try {
            ...
            // Map中没有这个key，value = new AtomicInteger()，返回null；有这个key，返回原值。
    ...
    /**
     * 告警信息上报监控系统
     * 1.从机器告警队列中take()数据；队列为空时，线程挂起等待队列中插入告警信息。
     * 2.告警任务数量-1
     * 3.发送告警信息到监控系统
